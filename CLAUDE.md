...
- `config.py` — pydantic models for `listen`'s config file (`commands` entries plus optional `language`/`source`), parsed from YAML/JSON via `yaml.safe_load`. Validation lives in the schema (strict `extra="forbid"`, duplicate-keyword check as a model validator), not hand-rolled checks; `load_config` converts YAML/validation errors to `RuntimeError` for the CLI's error path.
